...
$query_Recordset1 = "UPDATE item SET status=".$status." WHERE id=".$id;
$Recordset1 = mysql_query($query_Recordset1, $commreq) or die(mysql_error());

mysql_select_db($database_commreq, $commreq);
		$query_Recordset2 = "SELECT *, item.id AS itemid FROM item JOIN project ON item.projectid=project.id JOIN user ON project.user=user.id JOIN status ON item.status=status.id JOIN type ON item.type=type.id WHERE item.id = ".$id;
