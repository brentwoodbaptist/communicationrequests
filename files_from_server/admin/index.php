...
$query_Recordset1 = "SELECT *, item.id AS itemid, project.id AS projectid FROM item  JOIN project ON item.projectid=project.id JOIN type ON item.type=type.id JOIN user ON project.user=user.id JOIN department on project.department=department.id JOIN status ON item.status=status.id WHERE item.status=".$searchCriteria." ORDER BY ".$orderCriteria." LIMIT 1000";
...
<body>
		<header>
        	<div class="centercontent">
                ...
                <a href="bulletin.php" class="submitlink">Bulletin Only</a>
        	</div>
        </header>
        <nav id="subnav">
        	<div class="centercontent">
